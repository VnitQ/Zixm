...
usage() {
    cat <<'EOF'
Usage:
    bash BenchmarkMacOS.sh <test_path> <workers> [warmup] [runs] [label]
    bash BenchmarkMacOS.sh --help|-h
...
Examples:
    bash BenchmarkMacOS.sh llvm-project/llvm/test/CodeGen/X86 4
    bash BenchmarkMacOS.sh llvm-project/llvm/test/CodeGen/X86 4 8 20 baseline
EOF
}
...
check_cmd() {
    command -v "$1" &>/dev/null || { echo "ERROR: '$1' not found. brew install $2"; exit 1; }
}
