...
TEST(MicrosoftDemangle, demangleType) {
  ...
  EXPECT_EQ(microsoftDemangleString(".?AUStruct@@", MSDF_NoTypeDescription),
            "struct Struct");
  EXPECT_EQ(microsoftDemangleString(
                ".?AUStruct@@",
                MSDemangleFlags(MSDF_NoTagSpecifier | MSDF_NoTypeDescription)),
            "Struct");
}
...
TEST(MicrosoftDemangle, demangleVariable) {
  ...
  EXPECT_EQ(microsoftDemangleString("?globalVar@@3HA", MSDF_NoTypeDescription),
            "int globalVar");
}
