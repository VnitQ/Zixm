...
static uint64_t getCUAbbrev(StringRef Abbrev, uint64_t AbbrCode,
                            bool IsLittleEndian) {
  uint64_t Offset = 0;
  DataExtractor AbbrevData(Abbrev, IsLittleEndian, 0);
...
getIndexedString(dwarf::Form Form, DataExtractor InfoData, uint64_t &InfoOffset,
  ...
  DataExtractor StrOffsetsData(StrOffsets, InfoData.isLittleEndian(), 0);
  ...
  DataExtractor StrData(Str, InfoData.isLittleEndian(), 0);
...
getCUIdentifiers(InfoSectionUnitHeader &Header, StringRef Abbrev,
                 ...
                 bool IsLittleEndian) {
  DataExtractor InfoData(Info, IsLittleEndian, 0);
  ...
  DataExtractor AbbrevData(Abbrev, IsLittleEndian, 0);
  uint64_t AbbrevOffset = getCUAbbrev(Abbrev, AbbrCode, IsLittleEndian);
...
static Error addAllTypesFromTypesSection(
  ...
  for (StringRef Types : TypesSections) {
    Out.switchSection(OutputSection);
    uint64_t Offset = 0;
    DataExtractor Data(Types, IsLittleEndian, 0);
...
writeStringsAndOffsets(DWPWriter &Out, DWPStringPool &Strings,
  ...
  DataExtractor Data(CurStrSection, IsLittleEndian, 0);
  ...
  Data = DataExtractor(CurStrOffsetSection, IsLittleEndian, 0);
