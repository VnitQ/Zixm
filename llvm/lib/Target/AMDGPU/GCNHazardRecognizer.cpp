...
void GCNHazardRecognizer::AdvanceCycle() {
  ...
  if (IsVALUOrWMMA) {
    ...
    // (S_NOP-resolvable) hazard and schedules a non-VALU into those cycles, they
    // will not resolve the VALU-pipe hazard, so drop them here.
...
int GCNHazardRecognizer::getWaitStatesSinceVALU(IsHazardFn IsHazard,
  ...
  // window could miss a hazard. Keep the cap in sync with the wait-state tables.
...
int GCNHazardRecognizer::checkWMMACoexecutionHazards(MachineInstr *MI) const {
  ...
  // getWaitStatesSinceVALU checks for a hazard between instruction 'I' and 'MI':
