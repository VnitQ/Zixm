...
bool AAPointerInfoFloating::collectConstantsForGEP(
  ...
  auto Union = PtrOI;
  // Keep only the immediate predecessor as origin for the shifted ranges.
  Union.addToAll(ConstantOffset.getSExtValue());
  Union.setAllOrigins(*CurPtr);
...
ChangeStatus AAPointerInfoFloating::updateImpl(Attributor &A) {
  ...
  // Seed the associated pointer with its known offset, not an inferred
  // object extent. Access sizes are determined from the memory operations that
  // use the pointer, and pointer types do not constrain the underlying object.
  OffsetInfoMap[&AssociatedValue].insert(AA::RangeTy(0, 0), AssociatedValue);
  ...
  auto HandlePassthroughUser = [&](Value *Usr, Value *CurPtr, bool &Follow) {
    ...
    assert(PtrOI.isValid() && "Invalid input OffsetInfo");

    if (isa<PHINode>(Usr) || isa<SelectInst>(Usr)) {
      // Merge with an additional origin for the PHI/Select, preserving the
      // existing range/origin invariant inside OffsetInfo.
      UsrOI.mergeWithOffset(PtrOI, *CurPtr);
    } else {
      // Copy the pointer offset ranges and make CurPtr the sole source
      // origin for the copied ranges.
      UsrOI = PtrOI;
      UsrOI.setAllOrigins(*CurPtr);
    }
    Follow = true;
    return true;
  };
  ...
  auto UsePred = [&](const Use &U, bool &Follow) -> bool {
    ...
    if (auto *GEP = dyn_cast<GEPOperator>(Usr)) {
      ...
      auto &PtrOI = OffsetInfoMap[CurPtr];
      assert(PtrOI.isValid() && "Invalid input OffsetInfo");
      assert(UsrOI.isValid() && "Invalid user OffsetInfo");
...
struct AAAllocationInfoImpl : public AAAllocationInfo {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (NumBins == 0) {
      TypeSize NewAllocationSize = TypeSize::getFixed(0);
    ...
    TypeSize NewAllocationSize = TypeSize::getFixed(PackedRangeSize * 8);
  ...
  void dumpNewOffsetBins(raw_ostream &O) {
    O << "Printing Map from [OldOffsetsRange] : [NewOffsetsRange] if the "
         "offsets changed."
      << "\n";
    for (const auto &It : getNewOffsets()) {
...
private:
  ...
  /// Map old accessed ranges to their new ranges in the packed allocation.
  ...
  bool setNewOffsets(const AA::RangeTy &OldRange, int64_t OldOffset,
    ...
    AA::RangeTy NewRange(NewComputedOffset, Size);
    auto [It, Inserted] = NewComputedOffsets.try_emplace(OldRange, NewRange);
    if (Inserted)
      return true;
    if (It->second == NewRange)
      return false;
    It->second = NewRange;
    return true;
  }

  // A helper function to check if simplified values exist for the current
  // instruction. Right now we don't change the value and give up on modifying
  // the size and offsets of the allocation, but this may be sub-optimal.
  // TODO: handle case for a simplified value.
  ...
  bool checkIfAccessChainUsesMultipleBins(
    ...
    // Backtrack and check if any non-call instruction in the access-causing
    // chain accessed multiple byte ranges. Calls may have multiple bins from
    // different call arguments and are handled separately.
    SmallVector<Instruction *> ReadyList;
    SmallPtrSet<Instruction *, 8> Visited;
    ReadyList.push_back(LocalInst);
    while (!ReadyList.empty()) {
      Instruction *GetBack = ReadyList.pop_back_val();

      if (!Visited.insert(GetBack).second)
        continue;

      // TODO: handle when one non-call instruction has multiple bins.
      if (!isa<CallBase>(GetBack)) {
        auto It = OffsetInfoMap.find(GetBack);
        if (It != OffsetInfoMap.end() && It->second.Ranges.size() > 1)
          return true;
      }

      for (Use &U : GetBack->operands())
        if (Instruction *Ins = dyn_cast<Instruction>(U.get()))
