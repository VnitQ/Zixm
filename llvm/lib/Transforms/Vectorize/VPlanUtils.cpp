...
/// all lanes only the Replicate bit is set. The SingleScalar bit can be set on
/// Wide or Replicate recipes, which indicates that the recipe could be narrowed
/// to single-scalar if legal and profitable. For instructions not producing
/// values, like an assume or store, the bits talk about the inherent widening
/// of the recipe. Finally, there is a class of instructions that necessarily
/// take vector operands and produce a scalar result, like
...
static VPWideningInfo getWideningInfo(const VPRecipeBase &R) {
  ...
  case VPRecipeBase::VPScalarIVStepsSC:
    return VPWideningInfo::Replicate;
