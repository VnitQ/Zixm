...
//    "escaped" immediately.
//
// 2. Heap allocations: isHeapAllocation() identifies malloc/new-family calls.
...
bool isHeapAllocation(const CallBase *CB, const TargetLibraryInfo &TLI) {
  // Try the standard path first. In current MemoryBuiltins, isAllocationFn()
  // already recognizes operator new/new[] and alloc/realloc-style functions,
  // including modern IR annotated with allockind.
  if (isAllocationFn(CB, &TLI))
    return true;

  // Fallback: isAllocationFn() relies on the allockind attribute being present.
  // Hand-crafted IR or old bitcode may lack it even for standard functions.
  // This TLI-direct check catches those cases.
