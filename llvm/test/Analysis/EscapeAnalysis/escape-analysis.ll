...
; Calloc (another heap allocation family): not returned -> no escape; returned -> escape
define void @calloc_no_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'calloc_no_escape':
; CHECK: p escapes: no
  %p = call ptr @calloc(i64 4, i64 16)
  ret void
}

define ptr @calloc_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'calloc_escape':
; CHECK: p escapes: yes
  %p = call ptr @calloc(i64 4, i64 16)
  ret ptr %p
}
...
; Flat alloca slot: pointer stored into plain alloca ptr, loaded back, escaped to global.
; Exercises doesStoredPointerEscapeViaLoads on the simple (non-GEP) path.
define void @store_slot_load_global_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'store_slot_load_global_escape':
; CHECK: a escapes: yes
; CHECK: slot escapes: no
  %a = alloca i8, align 1
  %slot = alloca ptr, align 8
  store ptr %a, ptr %slot
  %p = load ptr, ptr %slot
  store ptr %p, ptr @GPtr
  ret void
}

; Flat alloca slot: loaded pointer stored back into same local slot — no escape.
define void @store_slot_load_local_no_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'store_slot_load_local_no_escape':
; CHECK: a escapes: no
; CHECK: slot escapes: no
  %a = alloca i8, align 1
  %slot = alloca ptr, align 8
...
; freeze result unused -> no escape (freeze is semantically passthrough)
define void @freeze_unused_no_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'freeze_unused_no_escape':
; CHECK: a escapes: no
  %a = alloca i8, align 1
  %frozen = freeze ptr %a
  ret void
...
; freeze result used only in icmp -> no escape
define void @freeze_icmp_no_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'freeze_icmp_no_escape':
; CHECK: a escapes: no
  %a = alloca i8, align 1
  %frozen = freeze ptr %a
  %cmp = icmp eq ptr %frozen, null
  ret void
}

; freeze result stored to global -> escape (the frozen copy is exposed)
define void @freeze_global_store_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'freeze_global_store_escape':
; CHECK: a escapes: yes
  %a = alloca i8, align 1
  %frozen = freeze ptr %a
  store ptr %frozen, ptr @GPtr
  ret void
}
...
; addrspacecast: result unused -> no escape (addrspacecast is passthrough)
define void @addrspacecast_unused_no_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'addrspacecast_unused_no_escape':
; CHECK: a escapes: no
  %a = alloca i32, align 4
...
; addrspacecast: chain back to addrspace(0), result stored to global -> escape
define void @addrspacecast_chain_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'addrspacecast_chain_escape':
; CHECK: a escapes: yes
  %a = alloca i32, align 4
  %c1 = addrspacecast ptr %a to ptr addrspace(1)
  %c0 = addrspacecast ptr addrspace(1) %c1 to ptr
  store ptr %c0, ptr @GPtr, align 8
  ret void
}
...
; atomicrmw: value operand (the pointer stored atomically) -> escape
define void @atomicrmw_ptr_val_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'atomicrmw_ptr_val_escape':
; CHECK: a escapes: yes
  %a = alloca i32, align 4
  %old = atomicrmw xchg ptr @GPtr, ptr %a seq_cst
  ret void
}

; atomicrmw: pointer operand (the location being modified) -> no escape
define void @atomicrmw_ptr_dest_no_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'atomicrmw_ptr_dest_no_escape':
; CHECK: a escapes: no
  %a = alloca ptr, align 8
  %old = atomicrmw xchg ptr %a, ptr null seq_cst
  ret void
}
...
; select between alloca and null: result stored to global -> escape
; (select is passthrough in CaptureTracking, same as phi)
define void @select_alloca_null_escape(i1 %c) {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'select_alloca_null_escape':
; CHECK: a escapes: yes
  %a = alloca i32, align 4
  %v = select i1 %c, ptr %a, ptr null
  store ptr %v, ptr @GPtr, align 8
  ret void
}

; select between alloca and null: result unused -> no escape
define void @select_alloca_null_no_escape(i1 %c) {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'select_alloca_null_no_escape':
; CHECK: a escapes: no
  %a = alloca i32, align 4
  %v = select i1 %c, ptr %a, ptr null
  ret void
...
; stemsFromStartStore + LiveOnEntry path:
; %slot is written on one CFG arm but NOT the other. The exit load's defining
; access is a MemoryPhi with two incoming edges: startDef (from %t) and
; LiveOnEntry (from entry, slot uninitialised). stemsFromStartStore walks
; backward, hits LiveOnEntry on one arm → SkipSuccessors (our guard), finds
; startDef on the other arm → returns true. %x escapes.
define void @memphi_liveentry_arm_escape(i1 %c) {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'memphi_liveentry_arm_escape':
; CHECK: x escapes: yes
; CHECK: slot escapes: no
entry:
  %x = alloca i8, align 1
  %slot = alloca ptr, align 8
  br i1 %c, label %t, label %merge
t:
  store ptr %x, ptr %slot
  br label %merge
merge:
  ; MemoryPhi: {startDef from %t, LiveOnEntry from entry}
  %p = load ptr, ptr %slot
  store ptr %p, ptr @GPtr
  ret void
}

; Same structure, but loaded value not stored anywhere → %x does not escape.
define void @memphi_liveentry_arm_no_escape(i1 %c) {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'memphi_liveentry_arm_no_escape':
; CHECK: x escapes: no
; CHECK: slot escapes: no
entry:
  %x = alloca i8, align 1
...
; ============================================================================ ;
; Deallocation functions (free, realloc)
; ============================================================================ ;

declare void @free(ptr nocapture)
declare noalias ptr @realloc(ptr, i64)

; free: argument is nocapture -> pointer does not escape
define void @free_no_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'free_no_escape':
; CHECK: p escapes: no
...
; realloc: the old pointer is passed as a regular (capturing) argument.
; old pointer escapes; new pointer is not stored anywhere -> no escape.
define void @realloc_old_ptr_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'realloc_old_ptr_escape':
; CHECK: p escapes: yes
; CHECK: p2 escapes: no
  %p = call ptr @malloc(i64 16)
  %p2 = call ptr @realloc(ptr %p, i64 32)
  ret void
}
...
; lifetime.start/end: both take nocapture ptr -> no escape
define void @lifetime_no_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'lifetime_no_escape':
; CHECK: a escapes: no
  %a = alloca i32, align 4
...
declare void @llvm.memset.p0.i64(ptr noalias nocapture writeonly, i8, i64, i1)

; memset on local alloca: dest is nocapture writeonly -> no escape
define void @memset_no_escape() {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'memset_no_escape':
; CHECK: a escapes: no
