...
define amdgpu_cs void @atomic_store_f32x2_unordered_agent_align4(<2 x float> %in, ptr addrspace(1) %out) {
...
; GFX12-NEXT:    global_store_b64 v[2:3], v[0:1], off
; GFX12-NEXT:    s_endpgm
  store atomic <2 x float> %in, ptr addrspace(1) %out syncscope("agent") unordered, align 4
  ret void
}

define amdgpu_cs void @atomic_store_f32x2_monotonic_agent_align4(<2 x float> %in, ptr addrspace(1) %out) {
...
; GFX12-NEXT:    global_store_b64 v[2:3], v[0:1], off scope:SCOPE_DEV
; GFX12-NEXT:    s_endpgm
  store atomic <2 x float> %in, ptr addrspace(1) %out syncscope("agent") monotonic, align 4
  ret void
}
