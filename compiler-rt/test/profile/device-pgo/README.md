...
- the host ROCm drain runtime `clang_rt.profile_rocm` (opt-in,
  `COMPILER_RT_BUILD_PROFILE_ROCM=ON`). On **Linux** this contains the HSA
  introspection drain (`InstrProfilingPlatformROCm.cpp`); on **Windows** the
  host-shadow drain (`InstrProfilingPlatformROCmWindows.cpp`);
