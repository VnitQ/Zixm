...
void bcopy1 (void) {
  ...
  bcopy(src, dst, 5); // expected-warning{{out-of-bound}}
                      // expected-warning@-1{{bcopy' will always over-read; source buffer has size 4, but size argument is 5}}
}
...
void nocrash_on_empty_struct_memcpy(void) {
  ...
  // expected-warning@-3 {{'memcpy' will always over-read; source buffer has size 0, but size argument is 2}}
  // expected-warning@-4 {{Memory copy function overflows the destination buffer}}
#endif
}
