...
static void foo() {
  ...
  // expected-warning@-1{{Call argument 'makeObj()' of 'someFunction' is a raw pointer to CheckedPtr capable type 'RefCountableAndCheckable'}}
}
...
struct WrapperObj {
  ...
  void foo() {
    consume(checked);
    consume(checkedRef);
    // expected-warning@-1{{Call argument 'this->checkedRef' of 'call_args_member::consume' is a raw reference to CheckedPtr capable type 'CheckedObj'}}
  }
  void bar(WrapperObj& other) {
    consume(other.checked);
    // expected-warning@-1{{Call argument 'other.checked' of 'call_args_member::consume' is a raw reference to CheckedPtr capable type 'CheckedObj'}}
  }
};
...
// expected-warning@-1{{Call argument 'makeObj()' of 'call_args_default::someFunction' is a raw pointer to CheckedPtr capable type 'RefCountableAndCheckable'}}
