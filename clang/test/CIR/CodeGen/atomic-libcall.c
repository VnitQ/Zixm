...
void load(struct Big *ptr) {
  ...
  // CIR:      %[[DEST_SLOT:.+]] = cir.alloca !rec_Big, !cir.ptr<!rec_Big>, ["b"] {alignment = 4 : i64}
  ...
  // CIR-NEXT: cir.call @__atomic_load(%[[SIZE]], %[[PTR_VOIDPTR]], %[[DEST_VOIDPTR]], %[[ORDER]]) : (!u64i, !cir.ptr<!void>, !cir.ptr<!void>, !s32i) -> ()

  // LLVM:      %[[DEST:.+]] = alloca %struct.Big, i64 1, align 4
  // LLVM:      %[[PTR:.+]] = load ptr, ptr %{{.+}}, align 8
  // LLVM-NEXT: call void @__atomic_load(i64 24, ptr %[[PTR]], ptr %[[DEST]], i32 0)

  // OGCG:      %[[DEST:.+]] = alloca %struct.Big, align 4
  // OGCG:      %[[PTR:.+]] = load ptr, ptr %{{.+}}, align 8
  // OGCG-NEXT: call void @__atomic_load(i64 noundef 24, ptr noundef %[[PTR]], ptr noundef %[[DEST]], i32 noundef 0)
}
...
void c11_load(_Atomic(struct Big) *ptr) {
  ...
  // CIR:      %[[DEST_SLOT:.+]] = cir.alloca !rec_Big, !cir.ptr<!rec_Big>, ["b", init] {alignment = 4 : i64}
  // CIR-NEXT: %[[TEMP_SLOT:.+]] = cir.alloca !rec_Big, !cir.ptr<!rec_Big>, ["atomic-temp"] {alignment = 4 : i64}
  // CIR:      %[[PTR:.+]] = cir.load align(8) %{{.+}} : !cir.ptr<!cir.ptr<!rec_Big>>, !cir.ptr<!rec_Big>
  // CIR-NEXT: %[[PTR_INTPTR:.+]] = cir.cast bitcast %[[PTR]] : !cir.ptr<!rec_Big> -> !cir.ptr<!cir.int<u, 192>>
  ...
  // CIR-NEXT: %[[ORDER:.+]] = cir.const #cir.int<0> : !s32i
  // CIR-NEXT: cir.call @__atomic_load(%[[SIZE]], %[[PTR_VOIDPTR]], %[[TEMP_VOIDPTR]], %[[ORDER]]) : (!u64i, !cir.ptr<!void>, !cir.ptr<!void>, !s32i) -> ()
  ...
  // LLVM:      %[[PTR:.+]] = load ptr, ptr %{{.+}}, align 8
  // LLVM-NEXT: call void @__atomic_load(i64 24, ptr %[[PTR]], ptr %[[TEMP_SLOT]], i32 0)
  ...
  // OGCG-NEXT: call void @__atomic_load(i64 noundef 24, ptr noundef %[[PTR]], ptr noundef %[[TEMP_SLOT]], i32 noundef 0)
  // OGCG-NEXT: call void @llvm.memcpy.p0.p0.i64(ptr align 4 %[[DEST_SLOT]], ptr align 4 %[[TEMP_SLOT]], i64 24, i1 false)
}
...
void store(struct Big *dest, struct Big *val) {
  ...
  // CIR-NEXT: %[[ORDER:.+]] = cir.const #cir.int<0> : !s32i
  // CIR-NEXT: cir.call @__atomic_store(%[[SIZE]], %[[DEST_VOIDPTR]], %[[VALUE_VOIDPTR]], %[[ORDER]]) : (!u64i, !cir.ptr<!void>, !cir.ptr<!void>, !s32i)

  // LLVM:      %[[DEST:.+]] = load ptr, ptr %{{.+}}, align 8
  // LLVM-NEXT: %[[VALUE:.+]] = load ptr, ptr %{{.+}}, align 8
  // LLVM-NEXT: call void @__atomic_store(i64 24, ptr %[[DEST]], ptr %[[VALUE]], i32 0)

  // OGCG:      %[[DEST:.+]] = load ptr, ptr %{{.+}}, align 8
  // OGCG-NEXT: %[[VALUE:.+]] = load ptr, ptr %{{.+}}, align 8
  // OGCG-NEXT: call void @__atomic_store(i64 noundef 24, ptr noundef %[[DEST]], ptr noundef %[[VALUE]], i32 noundef 0)
}
...
void c11_store(_Atomic(struct Big) *dest, struct Big *val) {
  ...
  // LLVM-NEXT: call void @__atomic_store(i64 24, ptr %[[DEST]], ptr %[[TEMP_SLOT]], i32 0)

  // OGCG:      %[[DEST:.+]] = load ptr, ptr %{{.+}}, align 8
  // OGCG-NEXT: %[[VALUE:.+]] = load ptr, ptr %{{.+}}, align 8
  // OGCG-NEXT: call void @llvm.memcpy.p0.p0.i64(ptr align 4 %[[TEMP_SLOT:.+]], ptr align 4 %[[VALUE]], i64 24, i1 false)
  // OGCG-NEXT: call void @__atomic_store(i64 noundef 24, ptr noundef %[[DEST]], ptr noundef %[[TEMP_SLOT]], i32 noundef 0)
}
